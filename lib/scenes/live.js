    ...
    initChannel: function () {
        ...
        channel.on('data', function (res) {
            ...
            res.forEach(function (data) {
                var content = data.content , team = data.team;
                if (content && team) {
                    if (content.indexOf(team) == -1) {
                        content = team + content;
                    }
                }
                grid.appendRow([data.time || "", data.score ? gameInfo.host + " " + data.score + " " + gameInfo.visiting : "", content], true);
            });
            if (!self.typing) {
                self.loading();
            }
